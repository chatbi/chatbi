"""Type validation for Python objects.

Refer to: https://github.com/microsoft/TypeChat/tree/main/python
"""
...
from chatbi.agent.validator.result import Failure, Result, Success
...
def _handle_error(validation_error: pydantic.ValidationError) -> Failure:
    ...
    for error in validation_error.errors(include_url=False):
        ...
        input = error["input"]
        error_string += (
            f"failed for value `{json.dumps(input)}` because:\n  {error['msg']}"
        )
        error_strings.append(error_string)

    if len(error_strings) > 1:
        failure_message = (
            "Several possible issues may have occurred with the given data.\n\n"
        )
    else:
        failure_message = ""
    failure_message += "\n".join(error_strings)
